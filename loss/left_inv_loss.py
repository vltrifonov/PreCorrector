...
@jsparse.sparsify
def left_inv_loss(L, Ainv):
    "L should be dense (and not batched since function is vmaped)"
    return jnp.square(jnp.linalg.norm((Ainv @ L) @ L.T - jnp.eye(Ainv.shape[0]), ord='fro'))

def compute_loss_left_inv(model, X, y, reduction=jnp.sum):
    ...
    loss = vmap(left_inv_loss, in_axes=(0, 0), out_axes=(0))(L, Ainv)
    return reduction(loss)

def compute_loss_left_inv_with_cond(model, X, y, repeat_step, reduction=jnp.sum):
