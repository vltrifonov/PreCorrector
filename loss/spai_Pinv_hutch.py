...
def spai_Pinv_hutch(Pinv, x, b):
    return jnp.square(jnp.linalg.norm(x - Pinv @ b, ord=2))

def compute_loss_spai_Pinv_hutch(model, X, y, reduction=jnp.mean):
    ...
    loss = vmap(spai_Pinv_hutch, in_axes=(0, 0, 0), out_axes=(0))(Pinv, X[4], X[2])
    return reduction(loss)

def compute_loss_spai_Pinv_hutch_with_cond(model, X, y, repeat_step, reduction=jnp.mean):
