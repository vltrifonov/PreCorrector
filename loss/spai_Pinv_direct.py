...
def spai_Pinv_direct(Ainv, Pinv):
    return jnp.square(jnp.linalg.norm(Ainv - Pinv, ord='fro'))

def compute_loss_spai_Pinv_direct(model, X, y, reduction=jnp.mean):
    ...
    L_dense = L.todense()
    Pinv = vmap(lambda L_: L_ @ L_.T, in_axes=(0), out_axes=(0))(L_dense)
    loss = vmap(spai_Pinv_direct, in_axes=(0, 0), out_axes=(0))(jnp.linalg.inv(X[0].todense()), Pinv)
    return reduction(loss)

def compute_loss_spai_Pinv_direct_with_cond(model, X, y, repeat_step, reduction=jnp.mean):
