...
def batched_cg_scipy(A, b, P=None, atol=1e-12, maxiter=1000):
    iters_ls = [[], [], [], []]
    time_ls = [[], [], [], []]
    ...
    for i in range(A.shape[0]):
        ...
        sol, res_i, time_i = cg_scipy(jBCOO_to_scipyCSR(A_i), b_i, P_i, atol=atol, maxiter=maxiter)        
...
def cg_scipy(A, b, P, atol, maxiter):
    ...
    solution, info = cg(A_loc, b_loc, M=P, callback=residuals_callback, rtol=0, atol=atol, maxiter=maxiter, x0=np.array(random.normal(random.PRNGKey(42), b_loc.shape)))
